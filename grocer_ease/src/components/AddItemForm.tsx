...
const AddItemForm: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await addItem({ name, quantity: parseInt(quantity), category, price: parseInt(price), completed: false });
        setName('');
        setQuantity('1');
        setCategory('');
        setPrice('');
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col justify-center items-center w-full bg-slate-300 px-4 py-2.5 shadow-md rounded-xl mb-5 gap-2.5">
            ...
            <input type="text" value={price} onChange={(e) => setPrice((e.target.value))} placeholder="Price" required className='w-1/3 md:w-1/6' />
